...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("microsoft/speecht5_tts", "SpeechT5ForTextToSpeech", "SpeechT5Processor", True, "float32", 0, "cuda:0", False, False, "*", 3002, "http://example.com", "admin", "password"),
...
def test_synthesize(
    ...
    url = f"http://{endpoint}:{port}/api/v1/synthesize"
...
def test_tts_pipeline(
    ...
    # Send a test request to the API
    url = f"http://{endpoint}:{port}/api/v1/tts_pipeline"
    headers = {"Content-Type": "application/json"}
    auth = (username, password) if username and password else None

    payload = {
        "text": "This is a test text for speech synthesis using the pipeline.",
    }
