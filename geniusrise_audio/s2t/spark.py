...
class SpeechToTextSpark(SpeechToTextInference):
    ...
    def transcribe_dataframe(
        ...
        distributor = TorchDistributor(local_mode=False, use_gpu=self.use_cuda)
    ...
    def initialize_torch_distributed(self, backend: str = "gloo", **kwargs):
        """
        Initialize PyTorch's distributed computing environment.

        Args:
            backend (str, optional): The backend to use for distributed processing. Defaults to 'gloo'.
            **kwargs: Additional arguments for initializing the process group.
        """
        init_process_group(backend=backend, **kwargs)
